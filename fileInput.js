function uploadFileDocument(
  ...
  var ret = openModal(
    "/execution.FormAction.do?action=uploadFile&frmId=" +
      frmId +
      "&frmParent=" +
      frmParent +
      "&inModal=" +
  ...
  ret.onclose = function() {
    document.getElementById("frmMain").action =
      "execution.FormAction.do?action=refresh&frmParent=" +
      frmParent +
      "&frmId=" +
      frmId +
      "&inModal=" +
...
function downloadFileDocument(frmId, frmParent, docId, inModal) {
  document.getElementById("docFrame").src =
    "execution.FormAction.do?action=download&frmId=" +
    frmId +
    "&frmParent=" +
    frmParent +
    "&inModal=" +
...
function downloadTMPDocument(index, frmId, frmParent, attId, inModal) {
  document.getElementById("docFrame").src =
    "execution.FormAction.do?action=downloadTMP&frmId=" +
    frmId +
    "&frmParent=" +
    frmParent +
    "&inModal=" +
...
function eraseFileDocument(
  ...
  document.getElementById("frmMain").action =
    "execution.FormAction.do?action=removeFile&frmId=" +
    frmId +
    "&attId=" +
    attId +
    "&docId=" +
...
function updateFileDocumentModal(
  index,
  frmId,
  frmParent,
  attId,
  ...
  var ret = openModal(
    "/execution.FormAction.do?action=updateFile&frmId=" +
      frmId +
      "&usrLock=" +
      usrLock +
      "&index=" +
...
function lockFileDocument(
  ...
  document.getElementById("frmMain").action =
    "execution.FormAction.do?action=lockFile&frmId=" +
    frmId +
    "&attId=" +
    attId +
    "&docId=" +
...
function showFileDocumentHistory(docId, inModal) {
  openModal(
    "/execution.FormAction.do?action=history&docId=" +
      docId +
      "&inModal=" +
      inModal,
    500,
...
function digitalSignDocument(
  ...
  if (doModal) {
    openModal(
      "/execution.FormAction.do?action=secretPhrase&frmId=" +
        frmId +
        "&fldId=" +
        fldId +
        "&inModal=" +
    ...
    var doAfter = function(rets) {
      if (rets != null) {
        ...
        document.getElementById("frmMain").action =
          "execution.FormAction.do?action=signField&onbeforeunload=" +
          onbeforeunload +
          "&frmId=" +
          frmId +
          "&inModal=" +
          ...
          rets +
          "&selTab=" +
          document.getElementById(samplesTabName).getSelectedTabIndex();
...
function viewFieldSigns(
  ...
  var rets = openModal(
    "/execution.FormAction.do?action=viewFieldSigns&frmId=" +
      frmId +
      "&index=" +
      index +
      "&inModal=" +
